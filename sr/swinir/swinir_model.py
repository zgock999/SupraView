"""
SwinIRモデルの定義
https://github.com/JingyunLiang/SwinIR の公式実装から移植

重要な修正: 公式実装と完全に一致させるために、モデル構造とパラメータを調整
"""
...
# SwinIRモデルタイプの定義
class SwinIRModelType(Enum):
    REAL_SR = auto()         # 実写画像超解像
    REAL_SR_LARGE = auto()   # 実写画像超解像（大規模モデル）
    CLASSICAL_SR = auto()    # 古典的超解像
    LIGHTWEIGHT_SR = auto()  # 軽量超解像
...
class PatchEmbed(nn.Module):
    ...
    Args:
        ...
        in_chans (int): 入力画像のチャネル数, デフォルト: 3
        embed_dim (int): 埋め込み次元数, デフォルト: 96
        norm_layer (nn.Module, optional): 正規化レイヤー, デフォルト: None
    """
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # 公式実装に合わせる: パッチ埋め込みにはConv2dを使わず、別の方法で処理
        # projプロパティは作成しない (公式実装と一致)
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        # ResidualGroupの出力は元のxと同じ形状を維持するように
        res = self.residual_group(x, x_size)
        # PatchUnembedを適用してテンソルを2D形式に変換
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        # x_sizeを使用して正しい空間次元を復元
        x = x.transpose(1, 2).view(B, self.embed_dim, x_size[0], x_size[1])  # B HW C -> B C H W
        return x
...
class Upsample(nn.Sequential):
    ...
    Args:
        scale (int): Scale factor. Supported 2^n and 3.
...
class SwinIR(nn.Module):
    ...
    Args:
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default False
        upscale (int): Upscale factor. Default 2
        img_range (float): Image range. Default 1.0
        upsampler (str): The upsampler. Default ''
        resi_connection (str): The convolutional block before residual connection. Default '1conv'
    """
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3, embed_dim=96,
        ...
        # 公式実装に合わせてパッチエンベッドを初期化（特にin_chansとembed_dimのパラメータに注意）
        self.patch_embed = PatchEmbed(
    ...
    def flops(self):
        ...
        if self.upsampler == 'pixelshuffle':
            flops += H * W * 3 * self.embed_dim * self.num_feat
...
# モデルパラメータ設定関数
def make_model(model_type, scale=4):
    """
    モデルを作成するユーティリティ関数
    
    Args:
        model_type: モデルタイプ（SwinIRModelType列挙型）
        scale: 拡大スケール係数
    
    Returns:
        作成されたSwinIRモデル
    """
    if model_type == SwinIRModelType.REAL_SR:
        # 実写画像超解像（標準モデル）
        model = SwinIR(
    ...
    elif model_type == SwinIRModelType.REAL_SR_LARGE:
        # 実写画像超解像（大規模モデル）
        model = SwinIR(
            upscale=scale, in_chans=3, img_size=64, window_size=8,
    ...
    elif model_type == SwinIRModelType.CLASSICAL_SR:
        # 古典的超解像モデル
        model = SwinIR(
            upscale=scale, in_chans=3, img_size=64, window_size=8,
            img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180,
    ...
    elif model_type == SwinIRModelType.LIGHTWEIGHT_SR:
        # 軽量超解像モデル
        model = SwinIR(
            upscale=scale, in_chans=3, img_size=64, window_size=8,
